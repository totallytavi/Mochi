...
module.exports = {
  ...
  run: async (client, interaction, options) => {
    if((await client.models.Setting.findOne({ where: { guildId: interaction.guild.id } })) === null) await client.models.Setting.create({ guildId: interaction.guild.id, autoVerify: false, addRoles: " ", removeRoles: " ", rolesRequired: 0, introChannel: " ", password: " " });
    ...
    if(subcommand === "view") {
      ...
      if(option.values[0] === "autoVerify") {
        ...
        client.models.Setting.update({ autoVerify: newValue.customId === "yes" ? true : false }, { where: { guildId: interaction.guild.id } });
        ...
        client.models.Setting.update({ password: password.first().content }, { where: { guildId: interaction.guild.id } });
        ...
        client.models.Setting.update({ addRoles: cleanRoles.join(",") }, { where: { guildId: interaction.guild.id } });
        ...
        client.models.Setting.update({ removeRoles: cleanRoles.join(",") }, { where: { guildId: interaction.guild.id } });
        ...
        await interaction.editReply({ content: "Please mention the channel to place the introduction in" });
        ...
        client.models.Setting.update({ introChannel: cleanChannel }, { where: { guildId: interaction.guild.id } });
        interactionEmbed(1, "", `Successfully set the introduction channel to: <#${cleanChannel}>`, interaction, client, [true, 20]);
        ...
        client.models.Setting.update({ rolesRequired: cleanNumber }, { where: { guildId: interaction.guild.id } });
